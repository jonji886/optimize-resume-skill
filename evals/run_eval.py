#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""optimize-resume 回归 eval runner。

用途：以后修改 SKILL.md / references / scripts 时，用固定 case 判断改动到底
让 Skill 变好还是变差，而不是只靠人工主观判断。

包含三个 suite：

    cases     高风险行为的回归测试（evals/cases/*.yaml）
...
用法:
    ...
    python3 evals/run_eval.py --case 001
...
class VariantResult:
    def __init__(self, case_id: str, name: str, expect: str) -> None:
...
def run_variant(case: Dict[str, Any], variant: Dict[str, Any]) -> VariantResult:
    ...
    result = VariantResult(case_id, str(variant.get("name") or "default"), expect)
...
def run_cases(filter_text: Optional[str]) -> Tuple[List[Dict[str, Any]], List[VariantResult]]:
    if yaml is None:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.suite in ("all", "cases"):
        cases, variant_results = run_cases(args.case)
    ...
    if args.suite in ("all", "cases"):
        ...
        for case_id in sorted(by_case):
            ...
            print(f"  {mark} {case_id:<42} {len(results) - len(bad)}/{len(results)} variants")
            for result in bad:
